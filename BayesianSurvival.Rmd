...
# 2. predefine a matrix
```{r, echo=TRUE}
n_samples = 1000
Betas = 3
Design_matrix = as.matrix(rbinom(n_samples, 1, prob = 0.5))
BaselinePara = 0.1


stan_data = SurvivalDataGenerator(n_samples, Betas, Design_matrix, 
                                  BaselinePara, BaselineDistr, BaselineDistr = "exponential")
...
```
# 3. build up survival models with covariates
```{r, echo=TRUE}
code_bayesian_survival_withCovariate_exponential <- "
  data { 
...
bayesian_survival_withCovariate <- stan_model(model_name = "bayesian_survival_withCovariate", 
                                         model_code = code_bayesian_survival_withCovariate_exponential)
...
```
# 4. build up survival models with continuous covariates
```{r, echo=TRUE}
